...
func GetJarvisUpdConfHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var req types.JarvisUpdConfReq
		if err := httpx.Parse(r, &req); err != nil {
			httpresp.HttpErr(w, r, errorx.NewStatCodeError(http.StatusBadRequest, 2, err.Error()))
			return
		}

		l := jarivsupd.NewGetJarvisUpdConfLogic(r.Context(), svcCtx)
		resp, err := l.GetJarvisUpdConf(req)
